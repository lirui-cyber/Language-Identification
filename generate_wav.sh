...
for x in lre17_train;do
    for y in 5 10 15 20;do
        ...
        cat ${data}/${x}_${y}_snrs/utt2lang | awk -v p=$path s=${snrs} '{l=length($1);name=substr($1,7,l);print name s" " p $1".wav"}' > ${data}/${x}_${y}_snrs/wav.scp
        cat ${data}/${x}_${y}_snrs/utt2lang | awk -v p=$path s=${snrs} '{l=length($1);name=substr($1,7,l);print name p" " $2}' > ${data}/${x}_${y}_snrs/utt2spk
